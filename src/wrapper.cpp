...
PYBIND11_MODULE(_pyzdd, m) {
    ...
    PyDdStructure2.def("cardinality", &tdzdd::DdStructure<2>::zddCardinality,
                       "count the number of sets in the family represented by "
                       "this ZDD. Returned type is str because cardinality may "
                       "be too enormous than int64.");
    ...
    py::class_<pyzdd::permutation::Permutation>(m, "Permutation")
        ...
        .def("__repr__", [](const pyzdd::permutation::Permutation &perm) {
            std::string repr = "(";
            for (pyzdd::permutation::Element i = 0, n = perm.get_size(); i < n;
                 ++i) {
                if (i != 0) {
                    repr += " ";
                }
    ...
    m.def("generate_permutation_group", &pyzdd::permutation::generate_group,
          py::arg("generators"));

    // structure enumeration
    m.def("construct_derivative_structures",
          &pyzdd::derivative_structure::construct_derivative_structures,
          py::arg("dd"), py::arg("num_sites"), py::arg("num_types"),
          py::arg("automorphism"),
          py::arg("translations") =
              std::vector<pyzdd::permutation::Permutation>(),
          py::arg("composition_constraints") = std::vector<int>(),
          py::arg("site_constraints") =
              std::vector<std::vector<pyzdd::permutation::Element>>(),
          py::arg("remove_incomplete") = false,
          py::arg("remove_superperiodic") = false);
    m.def("convert_to_labeling",
    ...
    py::class_<pyzdd::graph::GraphAuxiliary>(m, "GraphAuxiliary")
        ...
        .def("map_vertex_to_position",
             &pyzdd::graph::GraphAuxiliary::map_vertex_to_position);
    py::class_<pyzdd::graph::VertexGraphFrontierManager>(
        m, "VertexGraphFrontierManager")
        .def(py::init<const pyzdd::graph::Graph &>())
        .def(py::init<const pyzdd::graph::Graph &,
                      const std::vector<pyzdd::graph::Vertex> &>())
        .def("get_vertex_order",
             &pyzdd::graph::VertexGraphFrontierManager::get_vertex_order)
        .def("get_max_frontier_size",
             &pyzdd::graph::VertexGraphFrontierManager::get_max_frontier_size);
    m.def("get_vertex_order_by_bfs", &pyzdd::graph::get_vertex_order_by_bfs,
          py::arg("graph"));

    // SRO enumeration
    m.def("prepare_derivative_structures_with_sro",
    ...
    m.def("restrict_pair_correlation",
          ...
          py::arg("dd"), py::arg("num_sites"), py::arg("num_types"),
          py::arg("vertex_order"), py::arg("graph"), py::arg("target"));
    m.def("remove_superperiodic_structures",
          &pyzdd::derivative_structure::remove_superperiodic_structures,
          py::arg("dd"), py::arg("num_sites"), py::arg("num_types"),
          py::arg("vertex_order"), py::arg("translations"));
    m.def(
    ...
    m.def("convert_to_binary_labeling_with_graph",
          &pyzdd::derivative_structure::convert_to_binary_labeling_with_graph,
          py::arg("itr"), py::arg("converter"));

    // Specifications
    // Combination spec
    py::class_<tdzdd::DdSpecBase<pyzdd::combination::Combination, 2>>
        PyDdSpecBaseCombination(m, "DdSpecBaseCombination");
    py::class_<tdzdd::DdSpec<pyzdd::combination::Combination, int, 2>>
        PyDdSpecCombination(m, "DdSpecCombination", PyDdSpecBaseCombination);
    py::class_<pyzdd::combination::Combination> PyCombination(
        m, "Combination", PyDdSpecCombination);
    ...
    PyDdStructure2.def(
        "zddSubset",
        &tdzdd::DdStructure<2>::zddSubset<pyzdd::combination::Combination>);

    // Choice spec
    py::class_<tdzdd::DdSpecBase<pyzdd::choice::Choice, 2>> PyDdSpecBaseChoice(
        m, "DdSpecBaseChoice");
    py::class_<tdzdd::DdSpec<pyzdd::choice::Choice, int, 2>> PyDdSpecChoice(
        m, "DdSpecChoice", PyDdSpecBaseChoice);
    ...
    PyChoice.def(py::init<int, int, std::vector<int> &, bool>(),
                 ...
                 py::arg("n"), py::arg("k"), py::arg("v"),
                 py::arg("allow_more_than") = false);
    PyDdStructure2.def(
        "zddSubset", &tdzdd::DdStructure<2>::zddSubset<pyzdd::choice::Choice>);
}
